/*
 * Compiling:
 * g++ -L../../SenseHat/build -lsense-hat 04_led_matrix.cpp    \
...
int main()
{
  ...
  for(int z = 0; z < 10; z++) {
    ...
    // Show string message
    SendCommand(START, "LED_show_message_cpp");
    string message = "TEST";
    for (int i = 0; i < message.length(); i++) {
      showCharacter(senseHat, &message.at(i));
      usleep(1000000);
      senseHat.blank();
    }
    SendCommand(STOP, "LED_show_message_cpp");

    // Show an 'A' letter
    SendCommand(START, "LED_show_letter_cpp");
    showCharacter(senseHat, "A");
    usleep(3000000);
    SendCommand(STOP, "LED_show_letter_cpp");
    senseHat.blank();

    // Light up pixels for more accurate measurement.
    for (int constraint = 0; constraint <= 8; constraint++) {
      string measurement_message = "LED_set_pixels_cpp_" + to_string(constraint);
      SendCommand(START, measurement_message.c_str());
      ...
      usleep(3000000);
      SendCommand(STOP, measurement_message.c_str());
      senseHat.blank();
    }
  }
